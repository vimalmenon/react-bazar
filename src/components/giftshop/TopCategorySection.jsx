...
const TopCategorySection = ({
  ...
  return <Box>
      ...
      <Carousel totalSlides={categoryList.length} visibleSlides={visibleSlides} infinite={true} sx={{
      "& #backArrowButton, #backForwardButton": {
        ...
        {categoryList.map((item, ind) => <Link href="#" key={ind}>
            <a>
              <Box py={0.5}>
                <ProductCard15 title={item.title} available={item.available} imgUrl={item.imgUrl} />
              </Box>
            </a>
          </Link>)}
      </Carousel>
