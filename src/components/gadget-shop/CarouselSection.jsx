...
const CarouselSection = ({
  ...
  return <Box mt={-0.5} mb={-0.5}>
      <Carousel infinite={true} visibleSlides={visibleSlides} totalSlides={productList.length} leftButtonStyle={arrowButtonStyle} rightButtonStyle={arrowButtonStyle}>
        {productList.map((item, ind) => <ProductCard3 key={ind} id={item.id} imgUrl={item.imgUrl} title={item.name} rating={item.rating} price={item.price} off={item.discount} hideFavoriteIcon />)}
      </Carousel>
    </Box>;
};
