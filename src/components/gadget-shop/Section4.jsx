...
const Section4 = ({
  ...
  return <CategorySectionCreator title="Most Viewed">
      <Box mt={-0.5} mb={-0.5}>
        <Carousel totalSlides={mostViewedList.length} visibleSlides={visibleSlides} infinite={true} leftButtonStyle={arrowButtonStyle} rightButtonStyle={arrowButtonStyle}>
          {mostViewedList.map((item, ind) => <Box py={0.5} key={ind}>
              <ProductCard3 id={item.id} imgUrl={item.imgUrl} title={item.title} rating={item.rating} price={item.price} off={item.discount} />
            </Box>)}
        </Carousel>
      </Box>
